...
NONE = ["middle_finger", "blur", "four", "none", 'one', 'six', 'three', 'thumb']
...
def calculate_winner(move1, move2):
    ...
    if move1 == "middle_finger":
        return "Jerry says hi"
...
while True:
    ...
    now = time.time()
    if prev_move != user_move_name or now - prev_time >= 1.9:
        prev_time = time.time()
        move_step = 0
        if user_move_name not in NONE:
            computer_move_name = choice(['rock', 'paper', 'scissors'])
            winner = calculate_winner(user_move_name, computer_move_name)
        elif user_move_name == "middle_finger":
            computer_move_name = "mid"
            winner = "Jerry says hi"
        elif user_move_name == "thumb":
            computer_move_name = "thumb"
            winner = 'Great!'
        else:
    ...
    cv2.putText(frame, "Your Move: " + user_move_name,
                (150, 50), font, 1.2, (255, 0, 255), 2, cv2.LINE_AA)
    ...
    if start == 0:
        ...
        cv2.putText(frame, 'press s to start a game against computer',
                    (350, 700), font, 1, (0, 0, 0), 1, cv2.LINE_AA)

    if move_step >= 12 and start != 0:
        if user_move_name in ["rock", "paper", "scissors"]:
            if winner == "Tie":
                ...
                if start == 1:
                    cv2.putText(frame, 'you:' + str(you_win),
                                (200, 580), font, 2, (255, 0, 255), 4, cv2.LINE_AA)
